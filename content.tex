...
\State $\tau \rto \sum_{\GG^{ij}<0} (\GG^{ij})^2 / \sum_{\GG^{ij}<0} 1$ \Comment{variance of all negative local generalized correlations}
\State $\tau \rto \max\{0.01,\sqrt{\tau}\} \times 3.5$ \Comment{threshold based on negative correlations}
% \State $\tau_{2} \rto 2/n$
\State $\tau \rto \max\{\tau,2/n, \GG^{nn}\}$
\EndFunction
\end{algorithmic}
\end{algorithm}
...
The bibliography file has a relatively recent copy of everything jovo has ever cited, including~\citet{Vogelstein2019-om}, \url{https://jovo.me}.

% \lipsum[1-2]

Some general guidelines:
\begin{itemize}
    \item When using quotes, do not use the double quote symbol, rather use two \href{https://en.wikipedia.org/wiki/Grave_accent}{grave accent's} to start the quote, and two single quotes to end it.
    \item Put all figures in a 
    \begin{verbatim}
        \begin{figure}
    \end{verbatim}
    environment.
    \item Put all tables in a 
    \begin{verbatim}
        \begin{table}
    \end{verbatim}
    environment.
